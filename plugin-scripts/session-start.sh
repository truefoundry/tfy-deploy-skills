...
# Outputs JSON status that feeds back into the conversation context.
...
extract_version() {
  local ver_str="$1"
  # tfy --version may output "tfy 0.5.0" or just "0.5.0"
  echo "$ver_str" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1
}

install_tfy_cli() {
  # Attempt to install the TrueFoundry CLI; sets install_ok=true on success
  install_ok=false
  ...
  if $has_uv; then
    ...
    # Python 3.14+ needs pydantic beta for compatibility
...
if tfy_ver=$(tfy --version 2>/dev/null); then
  ...
  # Check version and auto-upgrade if needed
...
if [[ "$cred_status" = "ok" ]]; then
  ...
  if [[ "$http_code" =~ ^2 ]]; then
    ...
    # Parse workspace info from the response
...
if [[ "$cred_status" != "ok" ]]; then
  echo ""
  echo "Set credentials before deploying:"
